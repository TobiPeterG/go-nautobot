...
func (r ApiCircuitsCircuitTerminationsListRequest) AvailableForCable(availableForCable []string) ApiCircuitsCircuitTerminationsListRequest {
	r.availableForCable = &availableForCable
	return r
}
...
func (a *CircuitsAPIService) CircuitsCircuitTerminationsListExecute(r ApiCircuitsCircuitTerminationsListRequest) (*PaginatedCircuitTerminationList, *http.Response, error) {
	...
	if r.availableForCable != nil {
		t := *r.availableForCable
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "available_for_cable", s.Index(i).Interface(), "form", "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "available_for_cable", t, "form", "multi")
		}
	}
	if r.cable != nil {
		t := *r.cable
