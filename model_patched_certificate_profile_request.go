/*
API Documentation

Source of truth and network automation platform

API version: 3.0.1 (3.0)
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package nautobot

import (
	"encoding/json"
	"time"
)

// checks if the PatchedCertificateProfileRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PatchedCertificateProfileRequest{}

// PatchedCertificateProfileRequest CertificateProfile Serializer.
type PatchedCertificateProfileRequest struct {
	Id *string `json:"id,omitempty"`
	Name *string `json:"name,omitempty"`
	CertificateType *BulkWritableCertificateProfileRequestCertificateType `json:"certificate_type,omitempty"`
	CertificateFilePath *string `json:"certificate_file_path,omitempty"`
	ChainFilePath *string `json:"chain_file_path,omitempty"`
	KeyFilePath *string `json:"key_file_path,omitempty"`
	ExpirationDate NullableTime `json:"expiration_date,omitempty"`
	Cipher *string `json:"cipher,omitempty"`
	Tenant NullableApprovalWorkflowUser `json:"tenant,omitempty"`
	CustomFields map[string]interface{} `json:"custom_fields,omitempty"`
	Relationships *map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue `json:"relationships,omitempty"`
	Tags []ApprovalWorkflowStageResponseApprovalWorkflowStage `json:"tags,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _PatchedCertificateProfileRequest PatchedCertificateProfileRequest

// NewPatchedCertificateProfileRequest instantiates a new PatchedCertificateProfileRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPatchedCertificateProfileRequest() *PatchedCertificateProfileRequest {
	this := PatchedCertificateProfileRequest{}
	return &this
}

// NewPatchedCertificateProfileRequestWithDefaults instantiates a new PatchedCertificateProfileRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPatchedCertificateProfileRequestWithDefaults() *PatchedCertificateProfileRequest {
	this := PatchedCertificateProfileRequest{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *PatchedCertificateProfileRequest) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedCertificateProfileRequest) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *PatchedCertificateProfileRequest) SetId(v string) {
	o.Id = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *PatchedCertificateProfileRequest) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedCertificateProfileRequest) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *PatchedCertificateProfileRequest) SetName(v string) {
	o.Name = &v
}

// GetCertificateType returns the CertificateType field value if set, zero value otherwise.
func (o *PatchedCertificateProfileRequest) GetCertificateType() BulkWritableCertificateProfileRequestCertificateType {
	if o == nil || IsNil(o.CertificateType) {
		var ret BulkWritableCertificateProfileRequestCertificateType
		return ret
	}
	return *o.CertificateType
}

// GetCertificateTypeOk returns a tuple with the CertificateType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedCertificateProfileRequest) GetCertificateTypeOk() (*BulkWritableCertificateProfileRequestCertificateType, bool) {
	if o == nil || IsNil(o.CertificateType) {
		return nil, false
	}
	return o.CertificateType, true
}

// HasCertificateType returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasCertificateType() bool {
	if o != nil && !IsNil(o.CertificateType) {
		return true
	}

	return false
}

// SetCertificateType gets a reference to the given BulkWritableCertificateProfileRequestCertificateType and assigns it to the CertificateType field.
func (o *PatchedCertificateProfileRequest) SetCertificateType(v BulkWritableCertificateProfileRequestCertificateType) {
	o.CertificateType = &v
}

// GetCertificateFilePath returns the CertificateFilePath field value if set, zero value otherwise.
func (o *PatchedCertificateProfileRequest) GetCertificateFilePath() string {
	if o == nil || IsNil(o.CertificateFilePath) {
		var ret string
		return ret
	}
	return *o.CertificateFilePath
}

// GetCertificateFilePathOk returns a tuple with the CertificateFilePath field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedCertificateProfileRequest) GetCertificateFilePathOk() (*string, bool) {
	if o == nil || IsNil(o.CertificateFilePath) {
		return nil, false
	}
	return o.CertificateFilePath, true
}

// HasCertificateFilePath returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasCertificateFilePath() bool {
	if o != nil && !IsNil(o.CertificateFilePath) {
		return true
	}

	return false
}

// SetCertificateFilePath gets a reference to the given string and assigns it to the CertificateFilePath field.
func (o *PatchedCertificateProfileRequest) SetCertificateFilePath(v string) {
	o.CertificateFilePath = &v
}

// GetChainFilePath returns the ChainFilePath field value if set, zero value otherwise.
func (o *PatchedCertificateProfileRequest) GetChainFilePath() string {
	if o == nil || IsNil(o.ChainFilePath) {
		var ret string
		return ret
	}
	return *o.ChainFilePath
}

// GetChainFilePathOk returns a tuple with the ChainFilePath field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedCertificateProfileRequest) GetChainFilePathOk() (*string, bool) {
	if o == nil || IsNil(o.ChainFilePath) {
		return nil, false
	}
	return o.ChainFilePath, true
}

// HasChainFilePath returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasChainFilePath() bool {
	if o != nil && !IsNil(o.ChainFilePath) {
		return true
	}

	return false
}

// SetChainFilePath gets a reference to the given string and assigns it to the ChainFilePath field.
func (o *PatchedCertificateProfileRequest) SetChainFilePath(v string) {
	o.ChainFilePath = &v
}

// GetKeyFilePath returns the KeyFilePath field value if set, zero value otherwise.
func (o *PatchedCertificateProfileRequest) GetKeyFilePath() string {
	if o == nil || IsNil(o.KeyFilePath) {
		var ret string
		return ret
	}
	return *o.KeyFilePath
}

// GetKeyFilePathOk returns a tuple with the KeyFilePath field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedCertificateProfileRequest) GetKeyFilePathOk() (*string, bool) {
	if o == nil || IsNil(o.KeyFilePath) {
		return nil, false
	}
	return o.KeyFilePath, true
}

// HasKeyFilePath returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasKeyFilePath() bool {
	if o != nil && !IsNil(o.KeyFilePath) {
		return true
	}

	return false
}

// SetKeyFilePath gets a reference to the given string and assigns it to the KeyFilePath field.
func (o *PatchedCertificateProfileRequest) SetKeyFilePath(v string) {
	o.KeyFilePath = &v
}

// GetExpirationDate returns the ExpirationDate field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PatchedCertificateProfileRequest) GetExpirationDate() time.Time {
	if o == nil || IsNil(o.ExpirationDate.Get()) {
		var ret time.Time
		return ret
	}
	return *o.ExpirationDate.Get()
}

// GetExpirationDateOk returns a tuple with the ExpirationDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PatchedCertificateProfileRequest) GetExpirationDateOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return o.ExpirationDate.Get(), o.ExpirationDate.IsSet()
}

// HasExpirationDate returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasExpirationDate() bool {
	if o != nil && o.ExpirationDate.IsSet() {
		return true
	}

	return false
}

// SetExpirationDate gets a reference to the given NullableTime and assigns it to the ExpirationDate field.
func (o *PatchedCertificateProfileRequest) SetExpirationDate(v time.Time) {
	o.ExpirationDate.Set(&v)
}
// SetExpirationDateNil sets the value for ExpirationDate to be an explicit nil
func (o *PatchedCertificateProfileRequest) SetExpirationDateNil() {
	o.ExpirationDate.Set(nil)
}

// UnsetExpirationDate ensures that no value is present for ExpirationDate, not even an explicit nil
func (o *PatchedCertificateProfileRequest) UnsetExpirationDate() {
	o.ExpirationDate.Unset()
}

// GetCipher returns the Cipher field value if set, zero value otherwise.
func (o *PatchedCertificateProfileRequest) GetCipher() string {
	if o == nil || IsNil(o.Cipher) {
		var ret string
		return ret
	}
	return *o.Cipher
}

// GetCipherOk returns a tuple with the Cipher field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedCertificateProfileRequest) GetCipherOk() (*string, bool) {
	if o == nil || IsNil(o.Cipher) {
		return nil, false
	}
	return o.Cipher, true
}

// HasCipher returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasCipher() bool {
	if o != nil && !IsNil(o.Cipher) {
		return true
	}

	return false
}

// SetCipher gets a reference to the given string and assigns it to the Cipher field.
func (o *PatchedCertificateProfileRequest) SetCipher(v string) {
	o.Cipher = &v
}

// GetTenant returns the Tenant field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PatchedCertificateProfileRequest) GetTenant() ApprovalWorkflowUser {
	if o == nil || IsNil(o.Tenant.Get()) {
		var ret ApprovalWorkflowUser
		return ret
	}
	return *o.Tenant.Get()
}

// GetTenantOk returns a tuple with the Tenant field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PatchedCertificateProfileRequest) GetTenantOk() (*ApprovalWorkflowUser, bool) {
	if o == nil {
		return nil, false
	}
	return o.Tenant.Get(), o.Tenant.IsSet()
}

// HasTenant returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasTenant() bool {
	if o != nil && o.Tenant.IsSet() {
		return true
	}

	return false
}

// SetTenant gets a reference to the given NullableApprovalWorkflowUser and assigns it to the Tenant field.
func (o *PatchedCertificateProfileRequest) SetTenant(v ApprovalWorkflowUser) {
	o.Tenant.Set(&v)
}
// SetTenantNil sets the value for Tenant to be an explicit nil
func (o *PatchedCertificateProfileRequest) SetTenantNil() {
	o.Tenant.Set(nil)
}

// UnsetTenant ensures that no value is present for Tenant, not even an explicit nil
func (o *PatchedCertificateProfileRequest) UnsetTenant() {
	o.Tenant.Unset()
}

// GetCustomFields returns the CustomFields field value if set, zero value otherwise.
func (o *PatchedCertificateProfileRequest) GetCustomFields() map[string]interface{} {
	if o == nil || IsNil(o.CustomFields) {
		var ret map[string]interface{}
		return ret
	}
	return o.CustomFields
}

// GetCustomFieldsOk returns a tuple with the CustomFields field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedCertificateProfileRequest) GetCustomFieldsOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.CustomFields) {
		return map[string]interface{}{}, false
	}
	return o.CustomFields, true
}

// HasCustomFields returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasCustomFields() bool {
	if o != nil && !IsNil(o.CustomFields) {
		return true
	}

	return false
}

// SetCustomFields gets a reference to the given map[string]interface{} and assigns it to the CustomFields field.
func (o *PatchedCertificateProfileRequest) SetCustomFields(v map[string]interface{}) {
	o.CustomFields = v
}

// GetRelationships returns the Relationships field value if set, zero value otherwise.
func (o *PatchedCertificateProfileRequest) GetRelationships() map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue {
	if o == nil || IsNil(o.Relationships) {
		var ret map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue
		return ret
	}
	return *o.Relationships
}

// GetRelationshipsOk returns a tuple with the Relationships field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedCertificateProfileRequest) GetRelationshipsOk() (*map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue, bool) {
	if o == nil || IsNil(o.Relationships) {
		return nil, false
	}
	return o.Relationships, true
}

// HasRelationships returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasRelationships() bool {
	if o != nil && !IsNil(o.Relationships) {
		return true
	}

	return false
}

// SetRelationships gets a reference to the given map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue and assigns it to the Relationships field.
func (o *PatchedCertificateProfileRequest) SetRelationships(v map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue) {
	o.Relationships = &v
}

// GetTags returns the Tags field value if set, zero value otherwise.
func (o *PatchedCertificateProfileRequest) GetTags() []ApprovalWorkflowStageResponseApprovalWorkflowStage {
	if o == nil || IsNil(o.Tags) {
		var ret []ApprovalWorkflowStageResponseApprovalWorkflowStage
		return ret
	}
	return o.Tags
}

// GetTagsOk returns a tuple with the Tags field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedCertificateProfileRequest) GetTagsOk() ([]ApprovalWorkflowStageResponseApprovalWorkflowStage, bool) {
	if o == nil || IsNil(o.Tags) {
		return nil, false
	}
	return o.Tags, true
}

// HasTags returns a boolean if a field has been set.
func (o *PatchedCertificateProfileRequest) HasTags() bool {
	if o != nil && !IsNil(o.Tags) {
		return true
	}

	return false
}

// SetTags gets a reference to the given []ApprovalWorkflowStageResponseApprovalWorkflowStage and assigns it to the Tags field.
func (o *PatchedCertificateProfileRequest) SetTags(v []ApprovalWorkflowStageResponseApprovalWorkflowStage) {
	o.Tags = v
}

func (o PatchedCertificateProfileRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PatchedCertificateProfileRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.CertificateType) {
		toSerialize["certificate_type"] = o.CertificateType
	}
	if !IsNil(o.CertificateFilePath) {
		toSerialize["certificate_file_path"] = o.CertificateFilePath
	}
	if !IsNil(o.ChainFilePath) {
		toSerialize["chain_file_path"] = o.ChainFilePath
	}
	if !IsNil(o.KeyFilePath) {
		toSerialize["key_file_path"] = o.KeyFilePath
	}
	if o.ExpirationDate.IsSet() {
		toSerialize["expiration_date"] = o.ExpirationDate.Get()
	}
	if !IsNil(o.Cipher) {
		toSerialize["cipher"] = o.Cipher
	}
	if o.Tenant.IsSet() {
		toSerialize["tenant"] = o.Tenant.Get()
	}
	if !IsNil(o.CustomFields) {
		toSerialize["custom_fields"] = o.CustomFields
	}
	if !IsNil(o.Relationships) {
		toSerialize["relationships"] = o.Relationships
	}
	if !IsNil(o.Tags) {
		toSerialize["tags"] = o.Tags
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *PatchedCertificateProfileRequest) UnmarshalJSON(data []byte) (err error) {
	varPatchedCertificateProfileRequest := _PatchedCertificateProfileRequest{}

	err = json.Unmarshal(data, &varPatchedCertificateProfileRequest)

	if err != nil {
		return err
	}

	*o = PatchedCertificateProfileRequest(varPatchedCertificateProfileRequest)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "id")
		delete(additionalProperties, "name")
		delete(additionalProperties, "certificate_type")
		delete(additionalProperties, "certificate_file_path")
		delete(additionalProperties, "chain_file_path")
		delete(additionalProperties, "key_file_path")
		delete(additionalProperties, "expiration_date")
		delete(additionalProperties, "cipher")
		delete(additionalProperties, "tenant")
		delete(additionalProperties, "custom_fields")
		delete(additionalProperties, "relationships")
		delete(additionalProperties, "tags")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullablePatchedCertificateProfileRequest struct {
	value *PatchedCertificateProfileRequest
	isSet bool
}

func (v NullablePatchedCertificateProfileRequest) Get() *PatchedCertificateProfileRequest {
	return v.value
}

func (v *NullablePatchedCertificateProfileRequest) Set(val *PatchedCertificateProfileRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePatchedCertificateProfileRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePatchedCertificateProfileRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePatchedCertificateProfileRequest(val *PatchedCertificateProfileRequest) *NullablePatchedCertificateProfileRequest {
	return &NullablePatchedCertificateProfileRequest{value: val, isSet: true}
}

func (v NullablePatchedCertificateProfileRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePatchedCertificateProfileRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


