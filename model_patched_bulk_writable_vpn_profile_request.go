/*
API Documentation

Source of truth and network automation platform

API version: 3.0.1 (3.0)
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package nautobot

import (
	"encoding/json"
	"fmt"
)

// checks if the PatchedBulkWritableVPNProfileRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PatchedBulkWritableVPNProfileRequest{}

// PatchedBulkWritableVPNProfileRequest VPNProfile Serializer.
type PatchedBulkWritableVPNProfileRequest struct {
	Id string `json:"id"`
	Name *string `json:"name,omitempty"`
	Description *string `json:"description,omitempty"`
	KeepaliveEnabled *bool `json:"keepalive_enabled,omitempty"`
	KeepaliveInterval NullableInt32 `json:"keepalive_interval,omitempty"`
	KeepaliveRetries NullableInt32 `json:"keepalive_retries,omitempty"`
	NatTraversal *bool `json:"nat_traversal,omitempty"`
	// Additional options specific to the VPN technology and/or implementation
	ExtraOptions interface{} `json:"extra_options,omitempty"`
	Role NullableApprovalWorkflowUser `json:"role,omitempty"`
	SecretsGroup NullableApprovalWorkflowUser `json:"secrets_group,omitempty"`
	Tenant NullableApprovalWorkflowUser `json:"tenant,omitempty"`
	CustomFields map[string]interface{} `json:"custom_fields,omitempty"`
	Relationships *map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue `json:"relationships,omitempty"`
	Tags []ApprovalWorkflowStageResponseApprovalWorkflowStage `json:"tags,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _PatchedBulkWritableVPNProfileRequest PatchedBulkWritableVPNProfileRequest

// NewPatchedBulkWritableVPNProfileRequest instantiates a new PatchedBulkWritableVPNProfileRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPatchedBulkWritableVPNProfileRequest(id string) *PatchedBulkWritableVPNProfileRequest {
	this := PatchedBulkWritableVPNProfileRequest{}
	this.Id = id
	return &this
}

// NewPatchedBulkWritableVPNProfileRequestWithDefaults instantiates a new PatchedBulkWritableVPNProfileRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPatchedBulkWritableVPNProfileRequestWithDefaults() *PatchedBulkWritableVPNProfileRequest {
	this := PatchedBulkWritableVPNProfileRequest{}
	return &this
}

// GetId returns the Id field value
func (o *PatchedBulkWritableVPNProfileRequest) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *PatchedBulkWritableVPNProfileRequest) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *PatchedBulkWritableVPNProfileRequest) SetId(v string) {
	o.Id = v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *PatchedBulkWritableVPNProfileRequest) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedBulkWritableVPNProfileRequest) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *PatchedBulkWritableVPNProfileRequest) SetName(v string) {
	o.Name = &v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *PatchedBulkWritableVPNProfileRequest) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedBulkWritableVPNProfileRequest) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *PatchedBulkWritableVPNProfileRequest) SetDescription(v string) {
	o.Description = &v
}

// GetKeepaliveEnabled returns the KeepaliveEnabled field value if set, zero value otherwise.
func (o *PatchedBulkWritableVPNProfileRequest) GetKeepaliveEnabled() bool {
	if o == nil || IsNil(o.KeepaliveEnabled) {
		var ret bool
		return ret
	}
	return *o.KeepaliveEnabled
}

// GetKeepaliveEnabledOk returns a tuple with the KeepaliveEnabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedBulkWritableVPNProfileRequest) GetKeepaliveEnabledOk() (*bool, bool) {
	if o == nil || IsNil(o.KeepaliveEnabled) {
		return nil, false
	}
	return o.KeepaliveEnabled, true
}

// HasKeepaliveEnabled returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasKeepaliveEnabled() bool {
	if o != nil && !IsNil(o.KeepaliveEnabled) {
		return true
	}

	return false
}

// SetKeepaliveEnabled gets a reference to the given bool and assigns it to the KeepaliveEnabled field.
func (o *PatchedBulkWritableVPNProfileRequest) SetKeepaliveEnabled(v bool) {
	o.KeepaliveEnabled = &v
}

// GetKeepaliveInterval returns the KeepaliveInterval field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PatchedBulkWritableVPNProfileRequest) GetKeepaliveInterval() int32 {
	if o == nil || IsNil(o.KeepaliveInterval.Get()) {
		var ret int32
		return ret
	}
	return *o.KeepaliveInterval.Get()
}

// GetKeepaliveIntervalOk returns a tuple with the KeepaliveInterval field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PatchedBulkWritableVPNProfileRequest) GetKeepaliveIntervalOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.KeepaliveInterval.Get(), o.KeepaliveInterval.IsSet()
}

// HasKeepaliveInterval returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasKeepaliveInterval() bool {
	if o != nil && o.KeepaliveInterval.IsSet() {
		return true
	}

	return false
}

// SetKeepaliveInterval gets a reference to the given NullableInt32 and assigns it to the KeepaliveInterval field.
func (o *PatchedBulkWritableVPNProfileRequest) SetKeepaliveInterval(v int32) {
	o.KeepaliveInterval.Set(&v)
}
// SetKeepaliveIntervalNil sets the value for KeepaliveInterval to be an explicit nil
func (o *PatchedBulkWritableVPNProfileRequest) SetKeepaliveIntervalNil() {
	o.KeepaliveInterval.Set(nil)
}

// UnsetKeepaliveInterval ensures that no value is present for KeepaliveInterval, not even an explicit nil
func (o *PatchedBulkWritableVPNProfileRequest) UnsetKeepaliveInterval() {
	o.KeepaliveInterval.Unset()
}

// GetKeepaliveRetries returns the KeepaliveRetries field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PatchedBulkWritableVPNProfileRequest) GetKeepaliveRetries() int32 {
	if o == nil || IsNil(o.KeepaliveRetries.Get()) {
		var ret int32
		return ret
	}
	return *o.KeepaliveRetries.Get()
}

// GetKeepaliveRetriesOk returns a tuple with the KeepaliveRetries field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PatchedBulkWritableVPNProfileRequest) GetKeepaliveRetriesOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.KeepaliveRetries.Get(), o.KeepaliveRetries.IsSet()
}

// HasKeepaliveRetries returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasKeepaliveRetries() bool {
	if o != nil && o.KeepaliveRetries.IsSet() {
		return true
	}

	return false
}

// SetKeepaliveRetries gets a reference to the given NullableInt32 and assigns it to the KeepaliveRetries field.
func (o *PatchedBulkWritableVPNProfileRequest) SetKeepaliveRetries(v int32) {
	o.KeepaliveRetries.Set(&v)
}
// SetKeepaliveRetriesNil sets the value for KeepaliveRetries to be an explicit nil
func (o *PatchedBulkWritableVPNProfileRequest) SetKeepaliveRetriesNil() {
	o.KeepaliveRetries.Set(nil)
}

// UnsetKeepaliveRetries ensures that no value is present for KeepaliveRetries, not even an explicit nil
func (o *PatchedBulkWritableVPNProfileRequest) UnsetKeepaliveRetries() {
	o.KeepaliveRetries.Unset()
}

// GetNatTraversal returns the NatTraversal field value if set, zero value otherwise.
func (o *PatchedBulkWritableVPNProfileRequest) GetNatTraversal() bool {
	if o == nil || IsNil(o.NatTraversal) {
		var ret bool
		return ret
	}
	return *o.NatTraversal
}

// GetNatTraversalOk returns a tuple with the NatTraversal field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedBulkWritableVPNProfileRequest) GetNatTraversalOk() (*bool, bool) {
	if o == nil || IsNil(o.NatTraversal) {
		return nil, false
	}
	return o.NatTraversal, true
}

// HasNatTraversal returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasNatTraversal() bool {
	if o != nil && !IsNil(o.NatTraversal) {
		return true
	}

	return false
}

// SetNatTraversal gets a reference to the given bool and assigns it to the NatTraversal field.
func (o *PatchedBulkWritableVPNProfileRequest) SetNatTraversal(v bool) {
	o.NatTraversal = &v
}

// GetExtraOptions returns the ExtraOptions field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PatchedBulkWritableVPNProfileRequest) GetExtraOptions() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.ExtraOptions
}

// GetExtraOptionsOk returns a tuple with the ExtraOptions field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PatchedBulkWritableVPNProfileRequest) GetExtraOptionsOk() (*interface{}, bool) {
	if o == nil || IsNil(o.ExtraOptions) {
		return nil, false
	}
	return &o.ExtraOptions, true
}

// HasExtraOptions returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasExtraOptions() bool {
	if o != nil && !IsNil(o.ExtraOptions) {
		return true
	}

	return false
}

// SetExtraOptions gets a reference to the given interface{} and assigns it to the ExtraOptions field.
func (o *PatchedBulkWritableVPNProfileRequest) SetExtraOptions(v interface{}) {
	o.ExtraOptions = v
}

// GetRole returns the Role field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PatchedBulkWritableVPNProfileRequest) GetRole() ApprovalWorkflowUser {
	if o == nil || IsNil(o.Role.Get()) {
		var ret ApprovalWorkflowUser
		return ret
	}
	return *o.Role.Get()
}

// GetRoleOk returns a tuple with the Role field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PatchedBulkWritableVPNProfileRequest) GetRoleOk() (*ApprovalWorkflowUser, bool) {
	if o == nil {
		return nil, false
	}
	return o.Role.Get(), o.Role.IsSet()
}

// HasRole returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasRole() bool {
	if o != nil && o.Role.IsSet() {
		return true
	}

	return false
}

// SetRole gets a reference to the given NullableApprovalWorkflowUser and assigns it to the Role field.
func (o *PatchedBulkWritableVPNProfileRequest) SetRole(v ApprovalWorkflowUser) {
	o.Role.Set(&v)
}
// SetRoleNil sets the value for Role to be an explicit nil
func (o *PatchedBulkWritableVPNProfileRequest) SetRoleNil() {
	o.Role.Set(nil)
}

// UnsetRole ensures that no value is present for Role, not even an explicit nil
func (o *PatchedBulkWritableVPNProfileRequest) UnsetRole() {
	o.Role.Unset()
}

// GetSecretsGroup returns the SecretsGroup field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PatchedBulkWritableVPNProfileRequest) GetSecretsGroup() ApprovalWorkflowUser {
	if o == nil || IsNil(o.SecretsGroup.Get()) {
		var ret ApprovalWorkflowUser
		return ret
	}
	return *o.SecretsGroup.Get()
}

// GetSecretsGroupOk returns a tuple with the SecretsGroup field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PatchedBulkWritableVPNProfileRequest) GetSecretsGroupOk() (*ApprovalWorkflowUser, bool) {
	if o == nil {
		return nil, false
	}
	return o.SecretsGroup.Get(), o.SecretsGroup.IsSet()
}

// HasSecretsGroup returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasSecretsGroup() bool {
	if o != nil && o.SecretsGroup.IsSet() {
		return true
	}

	return false
}

// SetSecretsGroup gets a reference to the given NullableApprovalWorkflowUser and assigns it to the SecretsGroup field.
func (o *PatchedBulkWritableVPNProfileRequest) SetSecretsGroup(v ApprovalWorkflowUser) {
	o.SecretsGroup.Set(&v)
}
// SetSecretsGroupNil sets the value for SecretsGroup to be an explicit nil
func (o *PatchedBulkWritableVPNProfileRequest) SetSecretsGroupNil() {
	o.SecretsGroup.Set(nil)
}

// UnsetSecretsGroup ensures that no value is present for SecretsGroup, not even an explicit nil
func (o *PatchedBulkWritableVPNProfileRequest) UnsetSecretsGroup() {
	o.SecretsGroup.Unset()
}

// GetTenant returns the Tenant field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PatchedBulkWritableVPNProfileRequest) GetTenant() ApprovalWorkflowUser {
	if o == nil || IsNil(o.Tenant.Get()) {
		var ret ApprovalWorkflowUser
		return ret
	}
	return *o.Tenant.Get()
}

// GetTenantOk returns a tuple with the Tenant field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PatchedBulkWritableVPNProfileRequest) GetTenantOk() (*ApprovalWorkflowUser, bool) {
	if o == nil {
		return nil, false
	}
	return o.Tenant.Get(), o.Tenant.IsSet()
}

// HasTenant returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasTenant() bool {
	if o != nil && o.Tenant.IsSet() {
		return true
	}

	return false
}

// SetTenant gets a reference to the given NullableApprovalWorkflowUser and assigns it to the Tenant field.
func (o *PatchedBulkWritableVPNProfileRequest) SetTenant(v ApprovalWorkflowUser) {
	o.Tenant.Set(&v)
}
// SetTenantNil sets the value for Tenant to be an explicit nil
func (o *PatchedBulkWritableVPNProfileRequest) SetTenantNil() {
	o.Tenant.Set(nil)
}

// UnsetTenant ensures that no value is present for Tenant, not even an explicit nil
func (o *PatchedBulkWritableVPNProfileRequest) UnsetTenant() {
	o.Tenant.Unset()
}

// GetCustomFields returns the CustomFields field value if set, zero value otherwise.
func (o *PatchedBulkWritableVPNProfileRequest) GetCustomFields() map[string]interface{} {
	if o == nil || IsNil(o.CustomFields) {
		var ret map[string]interface{}
		return ret
	}
	return o.CustomFields
}

// GetCustomFieldsOk returns a tuple with the CustomFields field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedBulkWritableVPNProfileRequest) GetCustomFieldsOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.CustomFields) {
		return map[string]interface{}{}, false
	}
	return o.CustomFields, true
}

// HasCustomFields returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasCustomFields() bool {
	if o != nil && !IsNil(o.CustomFields) {
		return true
	}

	return false
}

// SetCustomFields gets a reference to the given map[string]interface{} and assigns it to the CustomFields field.
func (o *PatchedBulkWritableVPNProfileRequest) SetCustomFields(v map[string]interface{}) {
	o.CustomFields = v
}

// GetRelationships returns the Relationships field value if set, zero value otherwise.
func (o *PatchedBulkWritableVPNProfileRequest) GetRelationships() map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue {
	if o == nil || IsNil(o.Relationships) {
		var ret map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue
		return ret
	}
	return *o.Relationships
}

// GetRelationshipsOk returns a tuple with the Relationships field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedBulkWritableVPNProfileRequest) GetRelationshipsOk() (*map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue, bool) {
	if o == nil || IsNil(o.Relationships) {
		return nil, false
	}
	return o.Relationships, true
}

// HasRelationships returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasRelationships() bool {
	if o != nil && !IsNil(o.Relationships) {
		return true
	}

	return false
}

// SetRelationships gets a reference to the given map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue and assigns it to the Relationships field.
func (o *PatchedBulkWritableVPNProfileRequest) SetRelationships(v map[string]ApprovalWorkflowDefinitionRequestRelationshipsValue) {
	o.Relationships = &v
}

// GetTags returns the Tags field value if set, zero value otherwise.
func (o *PatchedBulkWritableVPNProfileRequest) GetTags() []ApprovalWorkflowStageResponseApprovalWorkflowStage {
	if o == nil || IsNil(o.Tags) {
		var ret []ApprovalWorkflowStageResponseApprovalWorkflowStage
		return ret
	}
	return o.Tags
}

// GetTagsOk returns a tuple with the Tags field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PatchedBulkWritableVPNProfileRequest) GetTagsOk() ([]ApprovalWorkflowStageResponseApprovalWorkflowStage, bool) {
	if o == nil || IsNil(o.Tags) {
		return nil, false
	}
	return o.Tags, true
}

// HasTags returns a boolean if a field has been set.
func (o *PatchedBulkWritableVPNProfileRequest) HasTags() bool {
	if o != nil && !IsNil(o.Tags) {
		return true
	}

	return false
}

// SetTags gets a reference to the given []ApprovalWorkflowStageResponseApprovalWorkflowStage and assigns it to the Tags field.
func (o *PatchedBulkWritableVPNProfileRequest) SetTags(v []ApprovalWorkflowStageResponseApprovalWorkflowStage) {
	o.Tags = v
}

func (o PatchedBulkWritableVPNProfileRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PatchedBulkWritableVPNProfileRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.KeepaliveEnabled) {
		toSerialize["keepalive_enabled"] = o.KeepaliveEnabled
	}
	if o.KeepaliveInterval.IsSet() {
		toSerialize["keepalive_interval"] = o.KeepaliveInterval.Get()
	}
	if o.KeepaliveRetries.IsSet() {
		toSerialize["keepalive_retries"] = o.KeepaliveRetries.Get()
	}
	if !IsNil(o.NatTraversal) {
		toSerialize["nat_traversal"] = o.NatTraversal
	}
	if o.ExtraOptions != nil {
		toSerialize["extra_options"] = o.ExtraOptions
	}
	if o.Role.IsSet() {
		toSerialize["role"] = o.Role.Get()
	}
	if o.SecretsGroup.IsSet() {
		toSerialize["secrets_group"] = o.SecretsGroup.Get()
	}
	if o.Tenant.IsSet() {
		toSerialize["tenant"] = o.Tenant.Get()
	}
	if !IsNil(o.CustomFields) {
		toSerialize["custom_fields"] = o.CustomFields
	}
	if !IsNil(o.Relationships) {
		toSerialize["relationships"] = o.Relationships
	}
	if !IsNil(o.Tags) {
		toSerialize["tags"] = o.Tags
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *PatchedBulkWritableVPNProfileRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varPatchedBulkWritableVPNProfileRequest := _PatchedBulkWritableVPNProfileRequest{}

	err = json.Unmarshal(data, &varPatchedBulkWritableVPNProfileRequest)

	if err != nil {
		return err
	}

	*o = PatchedBulkWritableVPNProfileRequest(varPatchedBulkWritableVPNProfileRequest)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "id")
		delete(additionalProperties, "name")
		delete(additionalProperties, "description")
		delete(additionalProperties, "keepalive_enabled")
		delete(additionalProperties, "keepalive_interval")
		delete(additionalProperties, "keepalive_retries")
		delete(additionalProperties, "nat_traversal")
		delete(additionalProperties, "extra_options")
		delete(additionalProperties, "role")
		delete(additionalProperties, "secrets_group")
		delete(additionalProperties, "tenant")
		delete(additionalProperties, "custom_fields")
		delete(additionalProperties, "relationships")
		delete(additionalProperties, "tags")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullablePatchedBulkWritableVPNProfileRequest struct {
	value *PatchedBulkWritableVPNProfileRequest
	isSet bool
}

func (v NullablePatchedBulkWritableVPNProfileRequest) Get() *PatchedBulkWritableVPNProfileRequest {
	return v.value
}

func (v *NullablePatchedBulkWritableVPNProfileRequest) Set(val *PatchedBulkWritableVPNProfileRequest) {
	v.value = val
	v.isSet = true
}

func (v NullablePatchedBulkWritableVPNProfileRequest) IsSet() bool {
	return v.isSet
}

func (v *NullablePatchedBulkWritableVPNProfileRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePatchedBulkWritableVPNProfileRequest(val *PatchedBulkWritableVPNProfileRequest) *NullablePatchedBulkWritableVPNProfileRequest {
	return &NullablePatchedBulkWritableVPNProfileRequest{value: val, isSet: true}
}

func (v NullablePatchedBulkWritableVPNProfileRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePatchedBulkWritableVPNProfileRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


